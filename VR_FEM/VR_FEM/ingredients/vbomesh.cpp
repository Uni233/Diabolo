                        ...
                        const vector<int> &elements )
{
    ...
    float3 * v = new float3[/*3 **/ nVerts * _nExternalMemory];
	memset(v,'\0',/*3 **/ nVerts * _nExternalMemory*sizeof(float3));LogInfo("v[%d]\n",(int)v);
    float3 * n = new float3[/*3 **/ nVerts * _nExternalMemory];
	memset(n,'\0',/*3 **/ nVerts * _nExternalMemory*sizeof(float3));LogInfo("n[%d]\n",(int)n);
    float2 * tc = NULL;
    float4 * tang = NULL;

    if(texCoords.size() > 0) {
        tc = new float2[ /*2 **/ nVerts * _nExternalMemory];
		memset(tc,'\0',/*2 **/ nVerts * _nExternalMemory*sizeof(float2));
        if( tangents.size() > 0 )
		{
            tang = new float4[/*4**/nVerts * _nExternalMemory];
			memset(tang,'\0',/*4 **/ nVerts * _nExternalMemory*sizeof(float4));
		}
    }
