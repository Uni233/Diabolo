...
#define DEBUG_sortVertexConnectedVertexSurface (0)
	...
	void vrBEM3D::distributeDof3d()
	{
		...
		/*const vrInt nVtxSize = Vertex::getVertexSize();
	...
	void vrBEM3D::renderScene() const
	{
		...
		for (iterAllOf(ci, sharedVec))
		{
			...
			//if (/*ptr->isDiscontinuous() &&*/ /*Mirror_Positive == ptr->getTriSetType()*/ 202 == ptr->getID())
			{
				...
				for (int i = 0; i < MyDim;++i)
				{
					...
					//std::cout << "elemDisp_DisContinuous[i] = " << elemDisp_DisContinuous[i].transpose() << std::endl;
				}

				for (int i = 0; i < MyDim;++i)
				{
					const MyVec3& vtx = ptr->getEndVtxPtr(i)->getPos();
					elemDisp_EndVtx[i] = ptr->m_data_SST_3D.interpolation_displacement(i, elemDisp_DisContinuous);

					//std::cout << "elemDisp_EndVtx[i] = " << elemDisp_EndVtx[i].transpose() << std::endl;
				...
				for (int i = 0; i < MyDim;++i)
				{
					...
					/*if (ptr->getVertex(i)->isContinuousVertex())
					{
					printf("vtx %d (%f,%f,%f)\n",ptr->getVertex(i)->getId(),vtx[0],vtx[1],vtx[2]);
					}*/
					//const vrFloat psn = ptr->getVertex(i)->getStressInfo();
					//curVtxColor = Colors::weightToColor( (psn - minStress) / (maxStress - minStress));
					//glColor3d(curVtxColor[0],curVtxColor[1],curVtxColor[2]);
...
#if 0
		{
			...
			for (int v=0;v<nVtxSize;++v)
			{
				...
				const VertexTypeInDual srcPtType = vtxPtr->getVertexTypeInDual();
		...
		{
		VertexPtr vtxPtr = Vertex::getVertex(v);
		if (vtxPtr->isDisContinuousVertex())
		{
		const MyVec3& vtx = vtxPtr->getPos();
		const MyVec3I& dofs = vtxPtr->getDofs();
		glVertex3d(vtx[0] + m_displacement[dofs[0]], vtx[1] + m_displacement[dofs[1]], vtx[2] + m_displacement[dofs[2]]);
	...
	void vrBEM3D::HG2A(const vrInt nDofs)
	{
		...
		
		{
			...
			for (int r=0;r<nRows;++r)
			{
				...
				{
					outfileH << r << " " << c << " " << m_Hsubmatrix.coeff(r,c) << std::endl;
				}
			}
			outfileH.close();
		}

		{
			...
			for (int r=0;r<nRows;++r)
			{
				for (int c=0;c<nCols;++c)
				{
					outfileG << r << " " << c << " " << m_Gsubmatrix.coeff(r,c) << std::endl;
				}
			}
			outfileG.close();
		}
...
#define DualEquation_Aliabadi_NegativeSection (1)
	...
	vrFloat vrBEM3D::compute_T_ij_I_SST_DisContinuous_Regular_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,vrInt idx_i,vrInt idx_j,vrInt idx_I)
	{
		vrFloat doubleLayer_Term = 0.0, singleLayer_Term = 0.0;
		const MyVec3 srcPos = curSourcePtr->getPos();
	...
	vrFloat vrBEM3D::compute_K_ij_I_SST_DisContinuous_Regular_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,const vrInt idx_i,const vrInt idx_j,const vrInt idx_I,const vrInt idx_k)
	{
#if 1
		vrFloat retVal = 0.0;
		vrFloat doubleLayer_Term_k = 0.0, singleLayer_Term_k = 0.0;
		const MyVec3 srcPos = curSourcePtr->getPos();
		const MyVec3 n_s = curSourcePtr->getVertexNormal();

		const vrInt nGaussPointSize_eta_In_Theta_SubTri_Regular = TriangleElem::GaussPointSize_eta_In_Theta_SubTri;
		const vrInt nGaussPointSize_eta_In_Rho_SubTri_Regular = TriangleElem::GaussPointSize_eta_In_Rho_SubTri;
	...
	vrFloat vrBEM3D::compute_S_ij_I_SST_DisContinuous_Regular_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,const vrInt idx_i,const vrInt idx_j,const vrInt idx_I,const vrInt idx_k)
	{
#if 1
		vrFloat retVal = 0.0;
		vrFloat doubleLayer_Term_k = 0.0, singleLayer_Term_k = 0.0;
...
vrFloat vrBEM3D::compute_T_ij_I_SST_DisContinuous_Sigmoidal_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,vrInt idx_i,vrInt idx_j,vrInt idx_I)
{
	vrFloat doubleLayer_Term = 0.0, singleLayer_Term = 0.0;
	const MyVec3 srcPos = curSourcePtr->getPos();
...
vrFloat vrBEM3D::compute_K_ij_I_SST_DisContinuous_Sigmoidal_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,const vrInt idx_i,const vrInt idx_j,const vrInt idx_I,const vrInt idx_k)
{
#if 0
...
vrFloat vrBEM3D::compute_S_ij_I_SST_DisContinuous_Sigmoidal_Aliabadi(const vrInt nSubTriIdx, const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D,const vrInt idx_i,const vrInt idx_j,const vrInt idx_I,const vrInt idx_k)
{
#if 0
	vrFloat retVal = 0.0;
	vrFloat doubleLayer_Term_k = 0.0, singleLayer_Term_k = 0.0;
	...
	vrFloat vrBEM3D::compute_S_ij_I_k_SST_Aliabadi(const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D, const vrInt idx_i, const vrInt idx_j, const  vrInt idx_I, const vrInt idx_k)
	{
		//MyError("vrBEM3D::compute_S_ij_I_k_SST_Aliabadi.");
		vrFloat retVal = 0.0;
#if 1
		...
		//for (int idx_k=0; idx_k < MyDim; ++idx_k)
		{
			...
			for (int index_theta=0,idx_g=0;index_theta<nGaussPointSize_xi_In_Theta;++index_theta)
			{
				...
				for (int index_rho=0;index_rho<nGaussPointSize_xi_In_Rho;++index_rho,++idx_g)
				{
					...
					const vrFloat SingularTerm_Sij_I_k = Sij_k * N_I * jacob_eta * theta_rho[TriangleElem::idx_rho_doubleLayer];
	...
	vrFloat vrBEM3D::compute_K_ij_I_k_SST_Aliabadi(const VertexPtr curSourcePtr, const TriangleElemData& refDataSST3D, const vrInt idx_i, const vrInt idx_j, const  vrInt idx_I, const vrInt idx_k)
	{
		...
		//for (int idx_k=0; idx_k < MyDim; ++idx_k)
		{
			...
			for (int index_theta=0,idx_g=0;index_theta<nGaussPointSize_xi_In_Theta;++index_theta)
			{
				...
				for (int index_rho=0;index_rho<nGaussPointSize_xi_In_Rho;++index_rho,++idx_g)
				{
					...
					const vrFloat SingularTerm_Kij_I_k = Kij_k * N_I * jacob_eta * theta_rho[TriangleElem::idx_rho_doubleLayer];
					const vrFloat SingularTerm_F_1_ij_I_k = (1.0/cur_rho)*(F_1_ij_I_k);
					doubleLayer_Term_k += (SingularTerm_Kij_I_k - SingularTerm_F_1_ij_I_k) * curWeight_doubleLayer;
