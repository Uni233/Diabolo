	...
	try{while (grid.activeVoxelCount() > 0) {
        ...
        while (!coordList.empty()) {
			...
			if(sourceAcc.isVoxel(ijk)){
				...
				if(!bndBox.empty()){
                    Coord n_ijks[6];
                    n_ijks[0]=bndBox.min_Coord() + Coord(-1, 0, 0);
                    n_ijks[1]=bndBox.min_Coord() + Coord( 0,-1, 0);
                    n_ijks[2]=bndBox.min_Coord() + Coord( 0, 0,-1);
                    n_ijks[3]=bndBox.max_Coord() + Coord( 1, 0, 0);
                    n_ijks[4]=bndBox.max_Coord() + Coord( 0, 1, 0);
                    n_ijks[5]=bndBox.max_Coord() + Coord( 0, 0, 1);
                    Coord span[12];
