...
namespace FractureSim{
    ...
    int VDBWrapper::init(
		...
		printf("nodes.size(%d) elems.size(%d) regions.size(%d) cracks.size(%d)\n",nodes.size(),elems.size(),regions.size(),cracks.size());
		system("pause");
		...
		{
			//
			openvdb::io::File file("d:/abc.vdb");
...
//			}
        ...
        : NBHW(nbHWidth)
    {
		...
		/*resXform->print();
		std::cout << "voxelSize_ " << voxelSize_ << std::endl;
		system("pause");*/
		/*index to world:
		[0.2, 0, 0, 0]
		[0, 0.2, 0, 0]
