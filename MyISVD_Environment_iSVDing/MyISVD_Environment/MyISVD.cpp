...
namespace YC
{
	...
	"""
	*/
	void SVD_updater::initial(const MyInt nDofs, bool update_V, const MyInt reorthStep)
	{
		m_outer_rows = (nDofs);
		n_rank_count = (0);
	...
	void SVD_updater::add_column(const MyVector& new_col)
	{
		...
		if (n_rank_count < m_outer_rows)
		{
			...
			if (mu < zero_epsilon) 
			{
				...
				_SVD_upd_diag(S, m_vec, false/*new_col = False*/, U1, S, V1);
			}
			else
			{
				//rank is increased
				rank_increases = true;
				concatenateVec(S,0.f);
				concatenateVec(m_vec, mu);
				_SVD_upd_diag(S, m_vec, true/*new_col = True*/, U1,S,V1);
				// Update outer matrix
	...
	*/
	void SVD_updater::_reorthogonalize()
	{
		if (!isNone(inner_U))
		{
			...
			Q_ASSERT(inner_U.cols() == S.size());
			MyMatrix US = inner_U * S.asDiagonal();
	...
	void SVD_updater::find_roots(const MyVector& sigmas, const MyVector& m_vec_addCols, MyVector& vecRoots)
	{
		...
		for (size_t i = 0; i < it_len; i++)
		{
			if (0 == i)
			{
				...
				root = one_root(sigmas[0], (sigmas[0] + std::sqrt(m_vec2.rowwise().sum()[0])), i, sigmas, m_vec, sig2, m_vec2);
			}
			else
			{
	...
	*/
		...
		bool new_col, bool & is_equal_sigmas, MyInt& uniq_length, MyMatrix& matU, MyVector& m_vec_transformed)
	{
		...
		MyVector z_col = m_vec;//z_col = m_vec.copy()
		...
		MyVector m_vec_abs = m_vec.cwiseAbs();
		...
		if (equal_inds_list.size() > 0)//# there are equal singular values and we need to do unitary transformation
		{
			...
			for (iterAllOf(iter, equal_inds_list))
			{
				...
				for (int i = 1 MyNotice; i < len_ll; ++i)//for i in xrange(1,len(ll)):
				{
					...
					MyFloat a = m; MyFloat b = m_vec[ll[i]];//a = m; b = m_vec[ ll[i] ]
	...
	*/
	void SVD_updater::_SVD_upd_diag(MyVector& sigmas, MyVector& m_vec, bool new_col, MyMatrix& matU, MyVector& vecS, MyMatrix& matV, int method/* = 2*/)
	{
		...
		for (int i = 0; i < new_sigmas.size();++i)
		{
			...
			{
				tmp1[k] = m_vec[k] / ((sigmas[k] - new_sigmas[k])*(sigmas[k] + new_sigmas[k]));
			}
	...
	*/
	void SVD_updater::update_SVD(MyMatrix& U, MyVector& S, MyMatrix& Vh, const MyVector& a_col, bool a_col_col)
	{
		...
		if (a_col_col) // if a_col_col: # new column
		{
			...
			if (mu < zero_epsilon) //if (np.abs(mu) < zero_epsilon) : # new column is from the same subspace as the old column
			{
				//# rank is not increased.
				//U1, new_sigmas, V1 = _SVD_upd_diag(S, m_vec, new_col = False)
				
				_SVD_upd_diag(S, m_vec, false/*new_col = False*/, U1, new_sigmas, V1);
				...
				MyVector tmpVec3(U1.rows()); tmpVec3.setZero();
				U1 = my_hstack(U1, tmpVec3);
				
				//V1 = np.hstack((V1, np.zeros((V1.shape[0], 1))))
				MyVector tmpVec4(V1.rows()); tmpVec4.setZero();
				V1 = my_hstack(V1, tmpVec4);
				//# <-
			}
			else
			{
				//U = np.hstack((U, new_u[:, np.newaxis] / mu))
				concatenateMat_OneColumn(U, new_u / mu);
				//S = np.concatenate((S, (0.0, )))
				S.conservativeResize(S.size() + 1);
				S[S.size() - 1] = 0.0;
				//m_vec = np.concatenate((m_vec, (mu, )))
				m_vec.conservativeResize(m_vec.size() + 1);
				m_vec[m_vec.size() - 1] = mu;
				//U1, new_sigmas, V1 = _SVD_upd_diag(S, m_vec, new_col = True)*/
				
				_SVD_upd_diag(S, m_vec, true, U1, new_sigmas, V1);
			}
...
/************************************************************************/
	...
	void test_SVD_updater()
	{
		...
		*/
		...
		const MyInt n_rows = 1000;
		const MyInt n_cols = 800;
