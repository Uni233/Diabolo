...
void Axis::initDraw()
{
    ...
    if (Lmax > Lmin*2 && Lmin > 0.0)
        Lmax = Lmin*2;
    if (Lmax > Lmin*2)
        Lmin = Lmax/(MySReal)1.414;
    Vector3 l(Lmin / (MySReal)50, Lmin / (MySReal)50, Lmin / (MySReal)50);
    Vector3 lc(Lmax / (MySReal)10, Lmax / (MySReal)10, Lmax / (MySReal)10); // = L / 5;
    Vector3 Lc = lc;
...
Axis::Axis(const Vector3& center, const Quaternion& orient, MySReal len)
{
    quadratic = NULL;
    length = Vector3(len,len,len);
    update(center, orient);
}
Axis::Axis(const Vector3& center, const double orient[4][4], MySReal len)
{
    quadratic = NULL;
    length = Vector3(len,len,len);
    update(center, orient);
...
Axis::Axis(const double *mat, MySReal len)
{
    quadratic = NULL;
    length = Vector3(len,len,len);
    update(mat);
...
void Axis::draw(const Vector3& center, const Quaternion& orient, MySReal len)
{
...
void Axis::draw(const Vector3& center, const double orient[4][4], MySReal len)
{
    Axis* a = get(Vector3(len,len,len));
    a->update(center, orient);
    a->draw();
...
void Axis::draw(const double *mat, MySReal len)
{
    Axis* a = get(Vector3(len,len,len));
    a->update(mat);
    a->draw();
...
void Axis::draw(const Vector3& p1, const Vector3& p2, const double& r1, const double& r2 )
{
    ...
    */
    ...
    q.elems = perp.elems.cross(n.elems);
    perp = n.cross(q);
